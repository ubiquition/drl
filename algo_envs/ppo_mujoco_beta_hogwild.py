...
# the value of max action
train_config['max_action'] = 1.0

# for tensorboard naming
train_config['tensorboard_comment'] = 'repeat_times_2'

class PPOMujocoBetaHogwildNet(AlgoBase.AlgoBaseNet):    
    """ Policy class used with continues PPO


    Example:
        >>> current_env_name = 'Ant' # Mujoco environment
        >>> train_net = PPOMujocoBetaHogwildNet()
    ...
    """
    def __init__(self):
        super(PPOMujocoBetaHogwildNet,self).__init__()
...
class PPOMujocoBetaHogwildUtils(AlgoBase.AlgoBaseUtils):
    pass
                                                                                                
class PPOMujocoBetaHogwildAgent(AlgoBase.AlgoBaseAgent):
    """
    Agent class used with continues PPO, allowing collect data and evaluate agents.

    Args:
        sample_net: policy network (default: PPOMujocoBetaHogwildNet)
    ...
    Example:
        ...
        >>> sample_agent = PPOMujocoBetaHogwildAgent(train_net,model_dict,is_checker=False)
        >>> transition = sample_agent.sample_env()
        >>> # Evaluating agent 
        >>> check_agent = PPOMujocoBetaShareAgent(train_net,model_dict,is_checker=True)
        >>> info = check_agent.check_env()
        >>> print(info['sum_rewards'], info['mean_entropys'], info['mean_mus'], info['mean_log_probs'])

    """
    def __init__(self,sample_net:PPOMujocoBetaHogwildNet,model_dict,is_checker):
        super(PPOMujocoBetaHogwildAgent,self).__init__()
        ...
        if not is_checker:
            ...
            print("PPOMujocoBetaHogwild check mujoco env is",env_name)
    ...
    def sample_env(self):
        ...
        for step in range(self.num_steps):
            ...
            for i in range(self.num_envs):
                ...
                exps[i].append([self.states[i],sample_acitons[i],reward_n,done,log_probs[i],self.model_dict['train_version']])
    ...
    def check_env(self):
        ...
        Example:
            >>> train_net = PPOMujocoBetaHogwildNet()
            >>> check_agent = PPOMujocoBetaHogwildAgent(train_net,model_dict,is_checker=True)
            >>> info = check_agent.check_env()
        """
...
class PPOMujocoBetaHogwildCalculate(AlgoBase.AlgoBaseCalculate):
    """
    training class used with continues PPO

    Args:
        share_model: policy network (default: PPOMujocoBetaHogwildNet)
        model_dict: a dict of model configuration
    ...
    Example:
        >>> train_net = PPOMujocoBetaHogwildNet()
        >>> calculate = PPOMujocoBetaHogwildCalculate(train_net,model_dict,calculate_index)
    ...
    """
    def __init__(self,share_model:PPOMujocoBetaHogwildNet,model_dict,calculate_index):
        super(PPOMujocoBetaHogwildCalculate,self).__init__()
        self.model_dict = model_dict
...
if __name__ == "__main__":
    ...
    agent = PPOMujocoBetaHogwildAgent(net,model_dict,is_checker=False)
    agent_check = PPOMujocoBetaHogwildAgent(net,model_dict,is_checker=True)
    calculate = PPOMujocoBetaHogwildCalculate(net,model_dict)
